...
async def register_user(db, user_data: UserCreate):
    ...
    user_dict["email"] = clean_email # Lưu email đã chuẩn hóa
...
async def login_user(db, login_data: UserLogin):
    ...
    if not user:
        raise HTTPException(status_code=404, detail="Email không tồn tại. Vui lòng kiểm tra lại.")
    ...
    return {
        ...
        "token_type": "bearer",
        "user_info": {
            "id": str(user["_id"]),  # BỔ SUNG ID ĐỂ FRONTEND SỬ DỤNG
            "name": user["name"],
            "email": user["email"],
            "role": user["role"]
        }
...
async def create_student_by_parent(db, parent_id: str, student_data: UserCreate):
    ...
    student_dict["email"] = clean_email # Lưu email đã chuẩn hóa
    ...
    return {"status": "success", "message": "Tạo tài khoản và liên kết thành công!", "student_id": new_student_id}