...
# --- THAO TÁC USER ---
st.write("---")
st.subheader(" Thao Tác Trực Tiếp")
if all_staff:
    if not search_query and selected_filter_role == "Tất cả":
        ...
        for idx, row in filtered_df.head(DISPLAY_LIMIT).iterrows():
            ...
            with st.expander(f" {row['name']} ({row['email']}) - {row['role_display']}"):
                tab1, tab2, tab3 = st.tabs([" Chỉnh Sửa", " Đổi Mật Khẩu", " Khóa / Xóa"])
                ...
                with tab3:
                    ...
                    if c_btn2.button(" Xóa Vĩnh Viễn", use_container_width=True, key=f"delete_{staff_id}"):
