...
tab_quiz, tab_video = st.tabs(["📝 Bộ Đề Quiz (Từ Database)", "🎬 AI-Video Hub (Từ Database)"])
    ...
    if not saved_quizzes:
        ...
        for i, q in enumerate(saved_quizzes):
            with st.container():
                st.markdown(f"""
                <div class="repo-card">
                    <h3 style="margin-top:0;">{q.get('title', 'Chưa có tên')}</h3>
                    <p style="margin-bottom:0;">🔢 Số câu: {len(q.get('questions', []))} | 📅 Ngày tạo: {q.get('created_at', 'Hôm nay')}</p>
                </div>
                """, unsafe_allow_html=True)
                
                c1, c2, c3 = st.columns([1, 1, 4])
                with c1:
                    if st.button("📤 Giao Đề Này", key=f"assign_{q.get('id', i)}", type="primary"):
                        st.session_state.selected_quiz_to_assign = q.get('title')
                        st.switch_page("pages/teacher/giao_bai.py")
                with c2:
                    # Tạm thời khóa nút Xóa vì Backend chưa viết API DELETE
                    st.button("🗑️ Xóa (Đang bảo trì API)", key=f"del_q_{q.get('id', i)}", disabled=True)
                st.write("---")
...
with tab_video:
    ...
    # FORM MÔ PHỎNG NHÂN VIÊN AI TẢI VIDEO LÊN DB THẬT
    with st.expander("🛠️ [Góc Mô Phỏng] Cổng Upload Video của Nhân viên AI"):
        ...
        with st.form("upload_video_form"):
            ...
            if st.form_submit_button("Tải Video Lên Hệ Thống (Lưu vào DB)"):
                if v_title and v_url:
                    ...
                    try:
                        # GỌI API POST ĐỂ LƯU XUỐNG DB
                        response = requests.post(API_URL_VIDEOS, json=payload)
                        if response.status_code in [200, 201]:
                            st.success("Nhân viên AI đã tải video thành công vào DB!")
                            time.sleep(1)
                            st.rerun()
                        else:
                            st.error("Lỗi khi lưu vào Database.")
                    except Exception:
                        st.error("Lỗi mất kết nối Backend!")
                else:
    ...
    if not ai_videos:
        st.info("Chưa có video AI nào trên hệ thống Database. (Hãy mở phần mô phỏng ở trên để tải lên 1 video thật nhé!)")
    else:
            ...
            with st.container():
                ...
                with v_col2:
                    ...
                    if st.button(f"👍 Thích Video Này", key=f"like_{vid.get('id', i)}", use_container_width=True):
                        st.success("Tính năng Like đang được cập nhật API!")
                    ...
                    with st.form(key=f"cmt_form_{vid.get('id', i)}"):
                        ...
                        if st.form_submit_button("Gửi bình luận"):
                            if new_cmt:
                                st.success("Tính năng Bình luận đang được cập nhật API!")
                st.write("---")